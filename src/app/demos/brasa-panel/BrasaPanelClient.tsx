...
export default function BrasaPanelClient({ lang }: { lang: DemoLang }) {
  ...
  const cycleItem = (key: string) => { const m = { ...stateRef.current.itemStage }; m[key] = ((m[key] || 0) + 1) % 3; setState({ itemStage: m }) }
  ...
  const advTableItem = (code: string, idx: number) => { const orders = readArr<Order>('brasa_orders'); const ix = orders.findIndex(x => x.code === code); if (ix < 0) return; const o = orders[ix]; if (o.status === 'entregado' || o.status === 'cancelado') return; const m = stages(); ensureStages(m, o); const station = (((o.items || [])[idx] || {}).cat || 'cocina') === 'bar' ? 'bar' : 'cocina'; const wasDone = stationAll2(o, m, station); const k = code + '#' + idx; m[k] = Math.min((m[k] || 0) + 1, 2); saveStages(m); const nowDone = stationAll2(o, m, station); const ns = syncStatus(o, m); const patch: Order = { ...o, status: ns }; if (ns === 'listo' && !patch.readyAt) patch.readyAt = Date.now(); if (ns !== 'listo') patch.readyAt = null; orders[ix] = patch; try { localStorage.setItem('brasa_orders', JSON.stringify(orders)) } catch { /* noop */ } if (nowDone && !wasDone) { logStationDone(o, station) } else { load() } }
  ...
  const chefAccept = (o: Order) => { const orders = readArr<Order>('brasa_orders'); const ix = orders.findIndex(x => x.code === o.code); if (ix < 0) return; const m = stages(); ensureStages(m, orders[ix]); (orders[ix].items || []).forEach((it, i) => { if ((it.cat || 'cocina') !== 'bar') { const k = o.code + '#' + i; if ((m[k] || 0) < 1) m[k] = 1 } }); saveStages(m); const staff = stateRef.current.staff; orders[ix] = { ...orders[ix], status: syncStatus(orders[ix], m), acceptedAt: Date.now(), acceptedBy: staff ? staff.name : '', cookId: (staff && staff.email) || '' }; try { localStorage.setItem('brasa_orders', JSON.stringify(orders)) } catch { /* noop */ } load() }
  ...
  const advItem = (key: string) => { const m = { ...stateRef.current.itemStage }; m[key] = Math.min((m[key] || 0) + 1, 2); setState({ itemStage: m }) }
  const resetItem = (key: string) => { const m = { ...stateRef.current.itemStage }; m[key] = 0; setState({ itemStage: m }) }
  ...
  const chargeOrder = (o: Order, method: string) => { const staff = stateRef.current.staff; const items = (o.items || []).map(i => ({ qty: i.qty, name: i.name, cat: i.cat || 'cocina', price: (i.price || 0) * (i.qty || 1), guest: o.guest || o.customer || c.guest })); logEvent({ type: 'cobro', code: o.code, staff: staff ? staff.name : '', role: 'mesero', method, total: o.total || 0, customer: o.customer || c.guest, mode: o.mode || '', guests: 1, items }); closeService(o.code); setState({ tenderFor: null }) }
